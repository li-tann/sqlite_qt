...
public:
    parVitalInfo(){};
    parVitalInfo(QString _fp, QDate _dt, double _inc,QString _obt, QString _lk,double _lon, double _lat, QString _ss, QString _pol)
...
public:
    ...
    /// 插入数据vi到数据库中
    bool insert_data(parVitalInfo vi);
    /// 删除重复数据, 目前还不能用
    ...
    /// 筛选符合key='value'条件的数据, 写入vis中, 该功能只支持单一条件筛选 只支持where
    bool select_data(QString key, QString value, vector<parVitalInfo>& vis);
    /// 筛选符合filters[i].first='filters[i].second'条件的数据, 写入vis中, 该功能可支持多个筛选条件 只支持where
    bool select_data(map<QString, QString>filters, vector<parVitalInfo>& vis);
